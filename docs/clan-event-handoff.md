...
- Main plugin class: `src/main/java/dinkplugin/DinkPlugin.java`
- Config interface: `src/main/java/dinkplugin/DinkPluginConfig.java`
- Runtime config/import manager: `src/main/java/dinkplugin/SettingsManager.java`
- Webhook routing for most notifiers: `src/main/java/dinkplugin/notifiers/BaseNotifier.java`
- Webhook HTTP + screenshot processing: `src/main/java/dinkplugin/message/DiscordMessageHandler.java`
- Screenshot capture utility: `src/main/java/dinkplugin/util/Utils.java`
- Tests live under `src/test/java/dinkplugin`.
...
- Implemented clan event files: `src/main/java/dinkplugin/ClanEventManager.java` and `src/main/java/dinkplugin/ClanEventOverlay.java`.
...
`src/main/java/dinkplugin/ClanEventManager.java`
...
- Add a chat warning/success through `DinkPlugin` when an event auto-ends.

It is injected into `DinkPlugin`, initialized on startup, notified on config changes, and ticked from `DinkPlugin#onGameTick`.
...
`src/main/java/dinkplugin/ClanEventOverlay.java`
...
- Register/unregister it in `DinkPlugin` using `OverlayManager`.
- Use a top-left or top-right position so screenshots naturally include it.

It is registered and unregistered in `DinkPlugin`. It renders the manager-provided text at top left.
...
- `src/main/java/dinkplugin/notifiers/BaseNotifier.java`
  - Make event webhook override all normal routing.
- `src/main/java/dinkplugin/notifiers/ExternalPluginNotifier.java`
  - Check whether any URL-list path bypasses the override. Adjust only if needed.
- `src/main/java/dinkplugin/message/DiscordMessageHandler.java`
  - Only touch if overlay is not captured in screenshots or to add guaranteed image stamping.
- New: `src/main/java/dinkplugin/ClanEventManager.java`
- New: `src/main/java/dinkplugin/ClanEventOverlay.java`
- Tests under `src/test/java/dinkplugin` and/or `src/test/java/dinkplugin/notifiers`.
